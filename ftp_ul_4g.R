...
ftp_ul_4g <- read.table("/home/rjdinis/development/R/mk_lte/FTP_UL_4G_All_v2.csv", header=TRUE, fill=TRUE, sep=',')
...
ftp_ul_4g_ie <- select(ftp_ul_4g, RNC, Technology, DLBandWidth, RSRP=Serving.RSRP..dBm., RSRQ=Serving.RSRQ..dB., SNR=Serving.RS.SNR..dB., CQI=Rank1.CQI, AvgMCS, RB.UL=NumRBs, PuschTxPower, TxPower=Tx.Power..dBm., RxPower=Rx.Power..dBm., Throughput.UL.MAC=MAC.UL.Throughput..kb.sec., Throughput.UL.RLC=RLC.UL.Throughput..kb.sec., Throughput.UL.PDCP=PDCP.UL.Throughput..kb.sec., Throughput.UL.APP=App.Throughput.UL..kbit.s.)
...
h13 <- ggplot(ftp_ul_4g_filter, aes(x=RB.UL)) + geom_histogram(aes(fill = ..count..), na.rm = TRUE) + scale_fill_gradient("Count", low = "green", high = "red")
